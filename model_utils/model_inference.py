...
def output_visuals(vis_rows, batch_data, outputs, args, index):
    ...
    #added by HH
    #print('infer pred_masks_ ', pred_masks_)
    #print(len(pred_masks_))
    ...
    # added by HH
    #print(args.mask_thres)
    #print(pred_masks_)
    #print(pred_masks_.shape)
    ...
    for j in range(B):
        ...
        #added by HH
        pred_sep_wav = [None for n in range(N)]
        ...
        for n in range(N):
            ...
            #added by HH
            #pred_sep_wav[n] = istft_reconstruction(mag_sep, phase_mix[j, 0], hop_length=args.stft_hop, win_length=args.stft_frame)
            ...
            #HH added
            wavfile.write('../music_source/sep_s{}/sep_s{}_1.wav' .format(index, index), args.audRate, preds_wav[n])
...
def inference(netWrapper, loader, history, epoch, args, index):
    ...
    for i, batch_data in enumerate(loader):
        ...
        if len(vis_rows) < args.num_vis:
            output_visuals(vis_rows, batch_data, outputs, args, index)

    print('Inference done.')


def run_inference(mix_wav, image_cue, index):
    # arguments
    print(4444)
    parser = ArgParser()
    ...
    # inference mode
    inference(netWrapper, loader_val, history, 0, args, index)
